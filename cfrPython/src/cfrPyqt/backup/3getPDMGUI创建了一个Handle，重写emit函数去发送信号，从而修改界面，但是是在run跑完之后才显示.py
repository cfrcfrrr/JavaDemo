...
class CreateGetBomGui(QWidget, Handler):
    '''
    PDM GUI Class
    '''
...
#         Handler.emit(self, record)
    ...
    def writeProgressTb(self):
        self.progressTb.append("XXX")
...
#         self.progressTb.setText("中文2")
        ...
        runLogicChildTh.start() #如果定位为局部变量，则当父线程的函数结束，局部变量被删除，就会崩溃。声明为局部变量可解决。
        #如果子线程要操作主线程的界面，也会崩溃，但是如果是新开一个进程来操作这个界面就没问题

#         testObj = TestClass()
        self.logSignal.connect(self.writeProgressTb)
        global RUN_PER_LIST
        nowPer = 0
...
#     consoleLog = logging.StreamHandler(PDMGUIObj)
